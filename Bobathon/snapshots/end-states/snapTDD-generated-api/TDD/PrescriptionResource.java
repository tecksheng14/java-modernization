...
public class PrescriptionResource {
    ...
    public Response createPrescription(Map<String, Object> prescriptionData) {
        try {
            ...
            if (!medicineRepository.existsById(medicineId)) {
                return createErrorResponse(Response.Status.BAD_REQUEST, "Invalid medicine ID");
            }
            ...
            // Set provided fields
            prescription.setPatientName((String) prescriptionData.get("patientName"));
            prescription.setPatientId((String) prescriptionData.get("patientId"));
            prescription.setDoctorName((String) prescriptionData.get("doctorName"));
            prescription.setMedicineId(medicineId);
            prescription.setMedicineName(medicineRepository.getMedicineName(medicineId));
            prescription.setQuantity(getIntegerValue(prescriptionData.get("quantity")));
            prescription.setDosage((String) prescriptionData.get("dosage"));
            prescription.setNotes((String) prescriptionData.getOrDefault("notes", ""));
    ...
    public Response validatePrescription(@PathParam("id") String id) {
        ...
        if (!"PENDING".equals(prescription.getStatus())) {
            Map<String, String> error = new HashMap<>();
            error.put("error", "Prescription is not in PENDING status");
    ...
    private void validateRequiredFields(Map<String, Object> data) throws ValidationException {
        String[] requiredFields = {
            ...
            "medicineId", "quantity", "dosage"
        };
        
        for (String field : requiredFields) {
            if (!data.containsKey(field) || data.get(field) == null) {
                throw new ValidationException(
                    "Invalid prescription data: " + field + " is required");
            }
        }
    }
    ...
    private void validateQuantity(Map<String, Object> data) throws ValidationException {
        ...
        if (quantity <= 0) {
            throw new ValidationException(
                "Invalid prescription data: quantity must be a positive number");
        }
    }
